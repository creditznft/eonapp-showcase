...
const host = String(process.env.W624E_PROOF_HOST || '127.0.0.1');
const port = Number(process.env.W624E_PROOF_PORT || 4173);
const baseURL = `http://${host}:${port}`;
const viteBin = path.join(root, 'node_modules', '.bin', process.platform === 'win32' ? 'vite.cmd' : 'vite');
const proofScript = path.join(root, 'scripts', 'w624e-eonbot-orbit-browser-proof.mjs');
...
const waitForServer = async (url, timeoutMs = 30_000) => { const started = Date.now(); while (Date.now() - started < timeoutMs) { if (await isReady(url)) return; await new Promise((resolve) => setTimeout(resolve, 250)); } throw new Error(`W624E proof server did not become ready at ${url}.`); };
...
try {
  ...
  } else console.log(`[W624E proof] Reusing server at ${baseURL}.`);
