...
for (const [name, result] of validations) {
  if (!result?.ok) {
    console.error(`W751 ${name} failed`, result);
    process.exit(1);
  }
}
...
const syntaxFiles = [
  ...
  'assets/js/work-surface/eon-station-work-loop.js',
  ...
  'scripts/w751-productive-stations-real-work-loops-gate.mjs'
];
...
console.log('W751 LOCAL SOURCE PASS — HEADED DESKTOP/MOBILE PRODUCTIVE-STATION VISUAL EVIDENCE STILL REQUIRED');
