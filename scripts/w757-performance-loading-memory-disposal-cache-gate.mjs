...
for (const [name, result] of [
  ...
  if (!result?.ok) { console.error(`W757 ${name} failed`, result); process.exit(1); }
}
...
const syntaxFiles = [
  ...
  'scripts/w757-performance-loading-memory-disposal-cache-gate.mjs'
];
...
console.log('W757 LOCAL SOURCE PASS — 20 DOCK/FOCUS CYCLES, 10 RESTARTS, CONTEXT RESTORE, LONG-SESSION MEMORY AND CACHE EVIDENCE STILL REQUIRE A REAL BROWSER');
