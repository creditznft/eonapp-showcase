...
const checks = [
  ...
  ['reward-live-safety', INFORMATIONAL_ROUTES.some((row) => row.from === '/rewards' && row.lifecycle === 'live-sensitive') && /Rewarded Sponsor Terminal/.test(rewards) && /qualifying server-validated completion adds exactly 1 Sponsor Key/.test(rewards) && /Reward issuance is server-authoritative and duplicate\/replay protected/.test(rewards) && /data-monetization="enabled"/.test(rewardsHtml) && /each qualifying server-validated completion grants exactly one consumable Sponsor Key/i.test(rewardsHtml)],
