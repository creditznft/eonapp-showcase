...
for (const [name, result] of validations) {
  if (!result?.ok) {
    console.error(`W754 ${name} failed`, result);
    process.exit(1);
  }
}

const syntaxFiles = [
  'assets/js/city/w754/eon-city-w754-cast-eonbot-npc-transit.js',
  ...
  'scripts/w754-cast-eonbot-npc-schedules-transit-gate.mjs'
];
...
console.log('W754 LOCAL SOURCE PASS — HEADED HIGH-DETAIL CAST, COLLISION, EONBOT CLEARANCE AND BOARD/SKIP EVIDENCE STILL REQUIRED');
