...
test('W252 Babylon scene consumes the art contract and retains procedural-only, local-only boundaries', () => {
  ...
  assert.match(scene, /original-art preview ready/);
  assert.doesNotMatch(`${scene}\n${art}`, /https?:|SceneLoader|AssetsManager|ImportMesh|fetch\s*\(|XMLHttpRequest|WebSocket|EventSource/);
