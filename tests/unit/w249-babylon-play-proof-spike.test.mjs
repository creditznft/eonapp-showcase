...
test('W249/W659K keep Babylon inside the authenticated entry action while canonical EON City enters directly', () => {
  ...
  assert.match(accessStation, /const core = await importCore\(\)/);
  ...
  assert.match(accessStation, /return import\('\.\/eon-city-play-core\.js'\)/);
  assert.doesNotMatch(accessStation, /from '\.\/eon-city-play-core\.js'/);
  // The older immersive station is retained for historical compatibility, but
  // the canonical entry now mounts the smaller verified core from the access
  // station. It must not become a second Babylon entry path.
...
test.skip('W249 Immersive Work Mode is full-screen and landscape-first with accessible touch controls and a safe fallback', () => {
  const station = read('assets/js/eon-city-play-station.js');
  const scene = read('assets/js/city/eon-city-play-babylon.js');
