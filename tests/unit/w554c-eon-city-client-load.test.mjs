...
test('W554C locks client-first static asset delivery and forbids Pages Function byte relays', () => {
  ...
  assert.equal(EON_CITY_CLIENT_DELIVERY.currentHeavyArtReleased, true);
  assert.equal(EON_CITY_CLIENT_DELIVERY.persistentAssetCacheName, 'eonapp-city-assets-v1');
  assert.equal(EON_CITY_CLIENT_DELIVERY.unchangedAssetsSurviveAppUpdates, true);
  assert.equal(EON_CITY_CLIENT_DELIVERY.logoutDoesNotClearAssetCache, true);
  assert.equal(EON_CITY_CLIENT_DELIVERY.lazyDistrictLoading, true);
...
test('W554C loading markup communicates direct delivery without false asset or user-data claims', () => {
  ...
  assert.match(markup, /cached on this device as it is opened/);
...
test('W554C extends the City access decision with client-first delivery truth', () => {
  ...
  assert.equal(decision.browserGateOnly, true);
  assert.equal(decision.edgeAssetProtectionConfigured, false);
  assert.equal(decision.edgeAssetProtectionRequiredBeforeBinaryArt, false);
});
