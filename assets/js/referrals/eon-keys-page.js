...
function sumAvailable(balances = {}) {
  return ['signal', 'builder', 'power'].reduce((total, key) => total + Number(balances?.available?.[key] || 0), 0);
}
...
function render(status = null) {
  ...
  const keyInventory = live.account?.balances?.available || { signal: 0, builder: 0, power: 0 };
  root.innerHTML = `
    <section class="eon-key-panel eon-key-hero" aria-labelledby="eon-key-hero-title">
      <p class="eon-key-kicker">Subscriptions · referrals · voluntary Sponsored Missions</p>
      <h1 id="eon-key-hero-title">Share useful work. Unlock more EONAPP.</h1>
      <p>Eligible referrals can earn referral EONKEYS. Separately, signed-in users can open voluntary MyLead Sponsored Missions from the <a href="/rewards">Reward Center</a> and earn numeric EONKEYS only after a trusted server postback confirms an eligible conversion.</p>
      <p><strong>No subscription replacement:</strong> EONKEYS never create cash value, renewal credit, provider credit or a whole-plan entitlement. Sponsored Mission EONKEYS redeem only for the bounded temporary software unlocks listed in the Reward Center.</p>
    ...
    <section class="eon-key-panel" aria-labelledby="eon-key-redeem-title"><h2 id="eon-key-redeem-title">Use an earned referral key</h2><p>Referral keys unlock their listed item through this referral ledger. Sponsored Mission EONKEYS use a separate numeric ledger and are redeemed in the <a href="/rewards">Reward Center</a>. Neither rail can become cash, a subscription or provider credit.</p><div class="eon-key-grid">${renderGrantRedemption(live)}</div></section>
    ...
    <section class="eon-key-panel" aria-labelledby="eon-key-types-title"><h2 id="eon-key-types-title">EONKEY types</h2><div class="eon-key-grid">${getEonKeyTypes().filter((key) => key.id !== 'sponsor').map(renderKey).join('')}</div></section>
    <section class="eon-key-panel" aria-labelledby="eon-key-matrix-title"><h2 id="eon-key-matrix-title">Verified referral reward path</h2><div class="eon-key-grid">${getEonReferralRewardMatrix().map(renderMatrix).join('')}</div><p class="eon-key-muted">Sponsored Missions are a separate voluntary path with their own numeric EONKEY ledger. Browser actions never mint rewards. <a href="/rewards">Open Reward Center</a>.</p></section>
