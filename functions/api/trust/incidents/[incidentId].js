...
import { updateIncident } from '../../../../assets/js/trust/eon-trust-support-ledger.js';
...
export async function onRequestPatch(context) {
  ...
  const incident = await updateIncident(context.env.EON_TRUST_DB, context.params.incidentId, parsed.value);
  return incident ? trustJson({ ok: true, incident }) : trustJson({ ok: false, error: 'incident_not_found_or_invalid_update' }, 404);
}
