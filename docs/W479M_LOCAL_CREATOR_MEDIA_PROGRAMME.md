...
## W477 Local Text setup bridge — already required before Creator Media

W477 begins the same non-technical onboarding pattern for **local text AI only** so a first-time user is never dropped into a blank “enter an Ollama endpoint” form. In EONBOT Chat, “Set up local AI” opens a short device-aware guide: choose a goal, receive one conservative text-model direction, tap an official runtime installer/model page, then return and explicitly scan plus self-test. It is a guidance layer, not installer automation, not a runtime scanner on page load, and not Local Creator Media.

The media programme inherits this UX rather than duplicating technical jargon: one primary route, visible alternatives, explain storage/device limits in ordinary language, and no media runtime/model execution until the separate adapter proof gate passes.
